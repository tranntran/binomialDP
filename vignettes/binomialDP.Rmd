...
pL = pnorm(x, mean = 0, sd = 1, lower.tail = FALSE)
pR = pnorm(x, mean = 0, sd = 1)
```
...
ppval = pvalLeft(tulap, n, 0.5, b, q)
...
abline(v=pL, col = "red")
legend("topright", 
       legend=c(paste("Non-private p-value =", round(pL,2)), 
...
```{r}
CILeft(alpha = 0.05, Z[1], size = n, b = b, q = q)
CIRight(alpha = 0.05, Z[1], size = n, b = b, q = q)
```
...
for (i in 1:length(Z)){
  CIone[i] = CILeft(alpha = 0.05, Z[i], size = n, b = b, q = q)
}
```
